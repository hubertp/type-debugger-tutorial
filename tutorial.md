...
The initial typechecking is only used for providing the users with a feedback about real compiler erros in the code and does not do the actual debugging. The latter is done using what we call [selective debugging](#SelectiveDebugging).

The user interface is divided into two parts: code editor on the right and visualization of the typechecking on the left. The initial view of the latter contains only a single node since we have not initiated any debugging yet.
Let's run `scalad` on a very simple program provided with the tutorial

    scalad resources/code/Show.scala

<!-- todo: include Show.scala -->

There we have two cases where we try to call a method `show` on an integer. For someone who relies on implicit conversions in every day programming it is obvious why `bar` will compile whereas `foo` will not. But how does the typechecker really makes such a decision? At which point does it finally fail? In the following sections we will show how to answer those questions.

### Selective debugging ### {#SelectiveDebugging}

In order to minimize the overhead of instrumentation and exploration of the typechecking proof type debugger provides a possibility of debugging only fragments of the code which are of interest to the user. Compiler will only display ant typecheck fragments necessary to satisfy that request (including those which are references but are not on the direct path). Hence the resulting proof tree will be a subset of the original one.
...
This reveals a fragment of the full proof tree explaining the typechecking steps done for checking that particular code fragment. Since we have an error within that particular method, the expanded proof tree will conveniently expand it up to the point at which compiler finally decided to give up and reported an error.
...
Basic goal information is succinct in order to reduce the amount of noise within the visualization. In order to navigate the tree it is useful to know that:

1. Hovering over goals will highlight corresponding code fragment in the editor (if possible).

2. Clicking on the goals will automatically expand their premises (if any). Clicking on an already expanded goal will collapse the details of its premises. Clicking bottom goals will always expand its parent.
...
1. Verify the signature of the tree
2. Type the tree.
3. Adapt the type of the tree to the expected type.
...
The resulting tree is almost ideal with a small exception: `Can we adapt qualifier to member and its arguments` goal now succeeds!. 

### Marking goals as *sticky*

It might be also convenient to select code trat covers both methods and compare each of the goals one by one (we leave it to the user to play around).
It is useful sometimes to mark the goal as *sticky* so that it does not collapse if we want to compare/leave it for further investigation. Just click the goal with `Cltr+Shift` (undoing the operation is done the same way). Error goals are typically *sticky* so you can treat them in the same way.
...
Type debugger is an excellent tool to explain the language in terms of the [Scala Language Specification](sls). You can enable it through `View goals -> Include Scala Language Specification information`. After that full-info tooltip will provide references to the document, if possible. Notice that at the moment references will be relatively sparse. We also noticed that for the purpose of type debugger, specification would have to provide better indexing for the sections - too be more precise.

## ... a little help would be nice ##
Although visualization of the typechecking process already explains decisions made by the compiler, a lot of an exploration burden is still on the shoulders of the user. This may be an acceptable cost if you are dealing with simple programs (like those until now) but would be discouraging for more realistic scenarios. 
...
Let's assume an example presented in **FindDefinition.scala**. Here our `Calculate` class relies on correct type inference for members `x` and `initial`. Since one of the assignments

{% highlight scala linenos %}
    x = calcSth(a, b)
{% endhighlight %}

leads to an error we would like to see what is wrong there. Selecting the fragment gives us a brief overview os the process of typechecking the assignment.

It seems that the assignee (or rather its setter) have a very weird type. At which point and how was this type inferred?
Finding out this information on the selected proof tree is already a tedious task. Therefore, let's go to the assignee goal `Typecheck assignee` of the assignment and right-click on `Can we find the definition of 'x'?` - an available option will trigger an expansion that leads to the point where variable's type was defined. 

For a similar example let's select the body of the `reset` method
...
This time the assignment is type correct but we would like to still find out exactly how does the type of `i` get inferred. We navigate to the first statement of the body and then to the signature verification goal `Can we verify signature of a value 'i'?`. But after two more expansions we are stuck since it gets the type `initial` by some magic in `Can we find the definition of 'initial'?`. Where and how do we get it? Let's use the provided helper once again and right-click on the node. The expanded sub proof is already non-trivial so the action reduces the burden of the proof search from the user.

#### Where did the initial error occur?
As you could see in the very first example 
...
Why is only the second example above incorrect? Let's expand proof for the last one first. Typechecking argument in the application has two premises: `Can we find the definition of 'x3'?` and `Can we adapt expression to the expected type A => Object?`. The latter hides some subgoals so let's right-click the correct action. That reveals the subtyping check `A => A <: A => Object` which succeeds (the algorithmic subtyping done here can become pretty large for more complicated types).

Let's now shift our focus on the application of `foo` to `x2`. Here again we have to adapt the type of the argument to the expected type but it fails. Let's right-click on the adaptation goal `Can we adapt expression to the expected type A => Object?` and ask it why. It should be obvious now that contravariance is the culprit here.

#### Where does this expected type come from?
Let's have a look again at the familiar `Calculate` example with the invalid assignment:
...
This reveals that we are in the middle of an application in the function body. For more information on how typechecker transforms such expressions feel free to roam around the goals with the toolkit but we will skip it for now. We are only interested where did this expected `Nil` type come from. So let's right click on the adaptation goal `Can we adapt expression to the expected type scala.collection.immutable.Nil.type?` and ask it.
The answer can be surprising because it is indeed our first `Nil` argument which affects the inferred type. By the time we apply our anonymous function to the second list of arguments all the type arguments are already inferred. With the visualization it is possible to understand the limitations of the current type inference.

#### Where do all these constraints come from?
In the previous example we were answering the question of where did the expected type come from. Type debugger can also help to understand how type arguments are inferred by the compiler. Our previous search highlighted goal `Given results from conformance (subtyping) tests with expected type, can type variables be finally resolved?`. Type variables are temporary types that collect constraints on the type parameters. However at some point we have to solve them and infer precise instantiations. The above goal has two children: `Can we solve type variable ?B ?` and the result of such type inference. Let's ask the former where do its constraints come from by right-clicking the goal. That reveals a close-by goal where the actual conformance (subtyping) check is done between the type of the argument and the formal parameter of the function at that position. In other words `Nil <: ?B` registers a lower bound constraint on the type variable representing type parameter `B`.
Finding the constraint source in this particular case would be very easy if done by hand but it is usually not the case for real programs.
